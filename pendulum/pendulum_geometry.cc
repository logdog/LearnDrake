#include "drake/logdog/pendulum/pendulum_geometry.h"
...
PendulumGeometry::PendulumGeometry(geometry::SceneGraph<double>* scene_graph) {
  ...
  // The base.
  {
    ...
    scene_graph->AssignRole(
        source_id_, id,
        MakePhongIllustrationProperties(Vector4d(.3, .6, .4, 1))); // color
  ...
  // The arm.
  {
    ...
    scene_graph->AssignRole(
        source_id_, id,
        MakePhongIllustrationProperties(Vector4d(.9, .1, .9, 1))); // color
    PerceptionProperties perception;
  ...
  // The mass at the end of the arm.
  {
    ...
    scene_graph->AssignRole(
        source_id_, id, MakePhongIllustrationProperties(Vector4d(0, 0, 1, 1))); // color
    PerceptionProperties perception;
